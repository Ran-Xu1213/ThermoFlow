...
def parse_pdb(pdb_path):
    ...
    for chain in model:
        ...
        sequence = []   
        coords = np.zeros((len(chain), 37, 3))
        mask = np.zeros((len(chain), 37), dtype=bool)
        ...
        for i, res in enumerate(chain):
            ...
            for atom in res:
                if atom.name in residue_constants.atom_order:
                    coords[i, residue_constants.atom_order[atom.name]] = [atom.pos.x, atom.pos.y, atom.pos.z]
                    mask[i, residue_constants.atom_order[atom.name]] = 1.0
                elif atom.name.upper() == "SE" and res_name == "MSE":
                
                    coords[i, residue_constants.atom_order["SD"]] = [atom.pos.x, atom.pos.y, atom.pos.z]
                    mask[i, residue_constants.atom_order["SD"]] = 1.0
        
        one_letter_seq = gemmi.one_letter_code(sequence)
  
        seq = np.array([restype_order_with_x.get(r, restype_order_with_x["X"]) for r in one_letter_seq], dtype=np.int32)
